...
```{r}
PPitest = simulPPi(Tmax,M1,alphatest,betatest)
par(mfrow=c(1,2))
...
for (Tmax in Tillustr){
  MLE_results=MLE(simulPPi(Tmax,Tmax,alpha,beta))
...
testintervallesalpha <- function(PPi, MLE_alpha, alpha) {
  ...
  for (i in seq_along(MLE_alpha)) {
    ...
    inside[i] <- (MLE_alpha[i] * exp(-log(t_i) / sqrt(N) * z)< MLE_alpha[i] && MLE_alpha[i] * exp(log(t_i) / sqrt(N) * z> MLE_alpha[i]))
  }
  return(mean(inside))
}
