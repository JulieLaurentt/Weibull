...
#### 2.1 Numerical simulations in order to validate, from a practical point of view, the confidence intervals presented in [Cocozza-Thivent, 1997]


Simulation d'un processus de poisson inhomogène :
```{r}
#simulPPi <- function(Tmax,M,alpha,beta)
#{ lambda_fct <- function(x){return(alpha/beta*(x/alpha)^(beta-1))}
 # t=cumsum(lambda_fct(0:Tmax))
 # n= simulPPh1(M,Tmax)
 # y=runif(n,0,M)
 # n[y<=lambda_fct(n)]
 # return(n)
...
Bien, maintenant que nous savons comment simuler un PPi, estimons les paramètres $\beta$ et $\alpha$ pour pouvoir faire des intervals de confiance.
...
testintervallesalpha <- function(PPi, MLE_alpha, alpha) {
  z <- qnorm(0.975)
  inside <- logical(length(MLE_alpha))
  for (i in seq_along(MLE_alpha)) {
    N=length(PPi)
    t_i <- PPi[i]  # on prend le t correspondant à alpha
    inside[i] <- ((MLE_alpha[i] * exp(-log(t_i) / sqrt(N) * z)> alpha) && (MLE_alpha[i] * exp(log(t_i) / sqrt(N) * z)<alpha))
    print(MLE_alpha[i] * exp(-log(t_i) / sqrt(N) * z))
    print(MLE_alpha[i] * exp(log(t_i) / sqrt(N) * z))
  }
  return (mean(inside))
}
...
testintervallesbeta <- function(PPi, MLE_beta, beta) {
  ...
  for (i in seq_along(MLE_beta)) {
    N=length(PPi)
    t_i <- PPi[i]  # on prend le t correspondant à beta
    inside[i] <- (MLE_beta[i]* (1 - z / sqrt(N)) < beta && MLE_beta[i] * (1 + z / sqrt(N)) > beta) 
  }
  return(mean(inside))
}
```
